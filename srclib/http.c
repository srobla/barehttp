...
int build_full_path(const char *root, http_request *request, char full_path[MAX_PATH]);

/* Gets size and last modification date given a path */
int get_file_info(const char *file_path, int *file_size, time_t *last_modified);
...
int send_file(char *full_path, int clientfd);
...
int process_script(const char *script_args, int args_len, char *full_path, int clientfd, server_conf *sc, log_file *lf);
...
int http_parse_request(char *buff, http_request *request, int buff_len, int last_len) {
    ...
    return phr_parse_request(buff, buff_len, &request->method, (size_t *)&request->method_size, &request->url, (size_t *)&request->url_size, &request->version, request->headers, (size_t *)&request->num_headers, last_len);    
}
...
int http_process_request(http_request *request, int clientfd, server_conf *sc, log_file *lf) {
    ...
    switch (request_method_type(request->method, request->method_size)) {
        case HTTP_METHOD_GET:
            logs_print(LOG_INFO, lf, "Received GET petition %.*s - client: %s", request->url_size, request->url,  ip_port_str);
            ret = process_get(request, clientfd, sc, lf);
            break;
        case HTTP_METHOD_POST:
            logs_print(LOG_INFO, lf, "Received POST petition %.*s - client: %s", request->url_size, request->url, ip_port_str);
            ret = process_post(request, clientfd, sc, lf);
            break;
        case HTTP_METHOD_OPTIONS:
            logs_print(LOG_INFO, lf, "Received OPTIONS petition %.*s - client: %s", request->url_size, request->url, ip_port_str);
            ret = process_options(clientfd, lf);
            break;
        default:
            logs_print(LOG_ERROR, lf, "Not supported method %d - client: %s", request->method, ip_port_str);
...
int http_get_content_length(http_request *request) {
    ...
    for (i = 0; i < request->num_headers; i++) {
        if (request->headers[i].name_len == 14 && memcmp(request->headers[i].name, "Content-Length", 14) == 0) {
...
void http_debug_request(http_request r){
    ...
    for (i = 0; i < r.num_headers; i++) {
        printf("\t%.*s: %.*s\n", (int)r.headers[i].name_len, r.headers[i].name, (int)r.headers[i].value_len, r.headers[i].value);
    }
    printf("Body: %.*s\n", r.body_size, r.body);
    printf("--------------------\n");
}
...
int process_post(http_request *request,int clientfd, server_conf *sc, log_file *lf){
    char full_path[MAX_PATH], *mime_type, ip_port_str[64];
    int i, ret;
    ...
    if(check_path_traversal(request->url, request->url_size)){
            logs_print(LOG_WARNING, lf, "Trying to access not allowed direction - client: %s", ip_port_str);
    ...
    /* build full path */
    ret = build_full_path(sc->server_root, request, full_path);
    if(ret == -2){
    ...
    if (strcmp(mime_type, "text/x-python") == 0 || strcmp(mime_type, "text/x-php") == 0){
        ret = process_script(request->body, request->body_size, full_path, clientfd, sc, lf);
        if (ret == -1) {
            return -1;
        }
        if (ret == -2) {
...
int process_script(const char *script_args, int args_len, char *full_path, int clientfd, server_conf *sc, log_file *lf) {
    ...
    int outpipe[2];
    pid_t pid;
    char buffer[MAX_BUFFR], script_out_buffer[MAX_BUFFR], date_str[MAX_HEADER_CONTENT], content_size_str[MAX_HEADER_CONTENT], ip_port_str[64];
    http_response response;
    ...
    ret = get_file_info(full_path, NULL, NULL);
    switch (ret) {
        case -2: 
            logs_print(LOG_WARNING, lf, "Requested script not exists - client: %s", ip_port_str);
            http_send_error(clientfd, HTTP_NOT_FOUND);
            return -2;
        case -3: 
            logs_print(LOG_WARNING, lf, "Requested script not allowed - client: %s", ip_port_str);
    ...
    while (total_bytes < args_len) {
        n_bytes = write(inpipe[1], script_args + total_bytes, args_len - total_bytes);
        if (n_bytes <= 0) break;
        total_bytes += n_bytes;
    }
    close(inpipe[1]);
    ...
    while (1) {
        ...
        n_bytes = read(outpipe[0], script_out_buffer + total_bytes, MAX_BUFFR - total_bytes);
        if (n_bytes <= 0) break;
...
int process_get(http_request *request, int clientfd, server_conf *sc, log_file *lf) {
    char full_path[MAX_PATH], buffer[MAX_BUFFR], *mime_type, ip_port_str[64];
    const char *script_args;
    int file_size, ret, args_len = 0, i;
    time_t last_modified;
    char date_str[MAX_HEADER_CONTENT], last_mod_str[MAX_HEADER_CONTENT], content_size_str[MAX_HEADER_CONTENT];
    time_t now = time(NULL);
    struct tm *time;
    http_response response;
    ...
    if(strcmp(mime_type, "text/x-python") == 0 || strcmp(mime_type, "text/x-php") == 0){
        ret = process_script(script_args, args_len, full_path, clientfd, sc, lf);
        if (ret == -2) return 0;
        if (ret == 0) {
            logs_print(LOG_INFO, lf, "Executing GET script - client: %s", ip_port_str);  
    ...
    /* read file */
    ret = get_file_info(full_path, &file_size, &last_modified);
    if(ret == -2){
        /* Not found */
        logs_print(LOG_WARNING, lf, "File %s not found - client: %s", full_path, ip_port_str);
        http_send_error(clientfd, HTTP_NOT_FOUND);
        return 0;
    }else if(ret == -3){
        /* Not allowed */
        logs_print(LOG_WARNING, lf, "File %s not allowed - client: %s", full_path, ip_port_str);
        http_send_error(clientfd, HTTP_FORBIDDEN);
        return 0;
    }else if(ret < 0){
        return -1;
    ...
    ret = strftime(date_str, sizeof(date_str), "%a, %d %b %Y %H:%M:%S GMT", time);
    if (ret == 0) {
        logs_print(LOG_ERROR, lf, "Error formatting actual date - client: %s", ip_port_str);
        http_send_error(clientfd, HTTP_INTERNAL_ERROR);
        return 0;
    }

    time = gmtime(&last_modified);
    ret = strftime(last_mod_str, sizeof(last_mod_str), "%a, %d %b %Y %H:%M:%S GMT", time);
    if (ret == 0) {
        logs_print(LOG_ERROR, lf, "Error formatting last modified date - client: %s", ip_port_str);
        http_send_error(clientfd, HTTP_INTERNAL_ERROR);
        return 0;
    }
    ...
    ret = tcp_send(clientfd, buffer, strlen(buffer));
    if (ret < 0) {
        logs_print(LOG_ERROR, lf , "Error sending HTTP response header - client: %s", ip_port_str);
        http_send_error(clientfd, HTTP_INTERNAL_ERROR);
        return 0;
    }

    ret = send_file(full_path, clientfd);
    if(ret < 0){
        return -1;
    }

    logs_print(LOG_INFO, lf, "Response HTTP, with file %s, sent - client: %s", full_path, ip_port_str);
...
int build_full_path(const char *root, http_request *request, char full_path[MAX_PATH]){
    ...
    if(request->url_size > MAX_PATH - 1) return -2;
    ...
    if(strlen(root) + strlen(path) >= MAX_PATH)
        return -2;

    memcpy(full_path, root, strlen(root));
    memcpy(full_path + strlen(root), path, strlen(path));
    full_path[strlen(root) + strlen(path)] = '\0';
